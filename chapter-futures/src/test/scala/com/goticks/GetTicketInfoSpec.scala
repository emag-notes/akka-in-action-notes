...
class GetTicketInfoSpec extends WordSpec with MustMatchers {
  ...
  "getTicketInfo" must {
    "return a complete ticket info when all futures are successful" in {
      ...
      ticketInfo.event.isDefined must be(true)
      ticketInfo.event.foreach(event => event.name must be("Quasimoto"))
      ticketInfo.travelAdvice.isDefined must be(true)
...
trait MockWebServiceCalls extends WebServiceCalls {
  ...
  def getEvent(ticketNr: String, location: Location): Future[TicketInfo] = Future {
    if (ticketNr == "1234") {
      TicketInfo(ticketNr,
                 location,
                 event =
  ...
  def callWeatherXService(ticketInfo: TicketInfo): Future[Option[Weather]] = Future {
    Some(Weather(30, precipitation = false))
  }

  def callWeatherYService(ticketInfo: TicketInfo): Future[Option[Weather]] = Future {
    Some(Weather(30, precipitation = false))
  }

  def callTrafficService(origin: Location, destination: Location, time: DateTime): Future[Option[RouteByCar]] = Future {
  ...
  def callPublicTransportService(origin: Location,
                                 ...
                                 time: DateTime): Future[Option[PublicTransportAdvice]] = Future {
    Some(PublicTransportAdvice("public transport route 1", time - 20.minutes, origin, destination, 20.minutes))
  }

  def callSimilarArtistsService(event: Event): Future[Seq[Artist]] = Future {
    Seq(Artist("Madlib", "madlib.com/calendar"),
        Artist("OhNo", "ohno.com/calendar"),
        Artist("Flying Lotus", "fly.lo/calendar"))
  }

  def callArtistCalendarService(artist: Artist, nearLocation: Location): Future[Event] = Future {
